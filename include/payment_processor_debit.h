...
struct PaymentProcessorDebit final : public PaymentProcessorAbstract {
  PaymentProcessorDebit(const NewOrder &new_order, std::string_view security_code)
  ...
  void DisplayInfo() const override {
    spdlog::info("Debit payment processor for order {0} with security code {1}", to_string(new_order_->GetId()), security_code_);
  }
